...
# Supondo que você já tenha definido a função LiquidDensityRackett em Python
    ...
    def calcular_propriedades_volumetricas(Tp, P, xl, Acentric, Tc, Pc, Vc, NC):
    # Chamar a função LiquidDensityRackett
    Vm, roLM, Vl, Cl, xLL = liquid_density_rackett(Tp, P, xl, Acentric, Tc, Pc, Vc, NC)
    
    VLM = Vm  # Volume molar da fase líquida [m³/mol]
    roLM = 1 / VLM  # Densidade molar da fase líquida [mol/m³]
    Vl = n / roLM  # Volume da fase líquida [m³]
    Cl = n / Vl  # Concentração da fase líquida [mol/m³]
    xLL = xl[1]  # Para impressão na saída
    
    return VLM, roLM, Vl, Cl, xLL

# Supondo que você tenha os valores necessários para as variáveis de entrada

# Chamar a função para calcular as propriedades volumétricas
VLM, roLM, Vl, Cl, xLL = calcular_propriedades_volumetricas(Tp, P, xl, Acentric, Tc, Pc, Vc, NC)
